...
class CreateTaskActivity : AppCompatActivity() {
    ...
    private fun showColorPickerDialog() {
        ...
        // 获取已使用的颜色，过滤出未使用的颜色
        val usedColors = dbHelper.getUsedColors()
        val availableColors = AVAILABLE_COLORS.filterNot { it in usedColors }

        if (availableColors.isEmpty()) {
            Toast.makeText(this, "所有颜色都已使用，请先删除一些任务", Toast.LENGTH_LONG).show()
            return
        }
    ...
    private fun setupViews() {
        ...
        binding.btnSave.setOnClickListener {
            ...
            if (id > 0) {
                Toast.makeText(this, "创建成功: $taskName, $duration", Toast.LENGTH_SHORT).show()
                finish()
            } else {
